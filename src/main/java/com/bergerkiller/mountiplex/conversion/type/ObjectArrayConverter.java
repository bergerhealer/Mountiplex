...
public class ObjectArrayConverter<T> extends BasicConverter<T> {
    ...
    public ObjectArrayConverter(Class<?> componentOutputType) {
        super((Class<T>) MountiplexUtil.getArrayType(componentOutputType));
    }
     ...
     * @param def value to return on failure (null, or an array of the component
     * type)
     ...
     * @return Array of the specified componentType containing the elements from
     * the collection
     */
    public <E> E[] convert(Collection<?> collection, Class<E> componentType) {
        if (componentType.isPrimitive()) {
            ...
            final E[] array = MountiplexUtil.createArray(componentType, length);
            final Iterator<?> iter = collection.iterator();
            for (int i = 0; i < length; i++) {
                array[i] = ConverterRegistry.convert(iter.next(), componentType, null);
            }
            return array;
        }
    }
    ...
    /**
     * Converts a primitive array (e.g. int[]) to an Object array of a requested
     * type (Double[])
     *
    ...
    public <E> E[] convertPrimitiveArray(Object primitiveArray, Class<E> componentType) {
        ...
        final E[] objectArray = MountiplexUtil.createArray(componentType, length);
        for (int i = 0; i < length; i++) {
            objectArray[i] = ConverterRegistry.convert(Array.get(primitiveArray, i), componentType, null);
        }
        return objectArray;
    }
