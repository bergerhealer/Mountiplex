...
public abstract class DuplexConverter<A, B> extends Converter<A, B> {
    ...
    private static final DuplexConverter.Supplier NONE_SUPPLIER = new Supplier() {
        public DuplexConverter get() { return null; }
        public boolean available() { return false; }
    };

    private static final class ConstantSupplier<A, B> implements Supplier<A, B> {
        private final DuplexConverter<A, B> result;
        public ConstantSupplier(DuplexConverter<A, B> result) { this.result = result; }
        public DuplexConverter<A, B> get() { return result; }
        public boolean available() { return true; }
    }

    /**
     * Produces a duplex converter when {@link #get()} is called
     * 
     * @param <A> Type A
     * @param <B> Type B
     */
    public static interface Supplier<A, B> {
        /**
         * Gets the DuplexConverter, returns null if not available
         * 
         * @return Duplex Converter
         */
        DuplexConverter<A, B> get();

        /**
         * Gets whether the duplex converter supplied by this supplier is available.
         * If this returns True, {@link #get()} is expected to return non-null.
         * 
         * @return True if available
         */
        default boolean available() { return get() != null; }

        /**
         * Combines the supplier for a converter from A to B and B to A into a Duplex Converter supplier.
         * Once {@link #get()} is called, both converter suppliers are queried.
         * 
         * @param converter
         * @param reverse
         * @return Supplier for a duplex converter A <> B
         */
        public static <A, B> Supplier<A, B> pair(Converter.Supplier<?, B> converter, Converter.Supplier<?, A> reverse) {
            return () -> DuplexConverter.pair(converter.get(), reverse.get());
        }

        /**
         * Gets a supplier for a constant value
         * 
         * @param duplexConverter
         * @return supplier that returns the duplexConverter
         */
        public static <A, B> Supplier<A, B> of(final DuplexConverter<A, B> duplexConverter) {
            return (duplexConverter == null) ? none() : (new ConstantSupplier<A, B>(duplexConverter));
        }

        /**
         * Gets a Supplier that always returns null (none, unavailable)
         * 
         * @return supplier that always returns null
         */
        @SuppressWarnings("unchecked")
        public static <A, B> Supplier<A, B> none() {
            return (Supplier<A, B>) NONE_SUPPLIER;
        }
    }
}
