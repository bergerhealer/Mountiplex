...
public interface GeneratedExactSignatureInvoker<T> extends Invoker<T> {
     ...
     */
    public static Class<? extends GeneratedExactSignatureInvoker<?>> generateInterface(MethodDeclaration methodDeclaration) {
        if (!methodDeclaration.isResolved()) {
            throw new IllegalArgumentException("Method declaration is not resolved: " + methodDeclaration);
        }

        MethodVisitor mv;
        ExtendedClassWriter<? extends GeneratedExactSignatureInvoker<?>> cw = ExtendedClassWriter.builder(GeneratedExactSignatureInvoker.class)
