...
public abstract class GeneratedConstructor implements Constructor<Object> {
    ...
    public static GeneratedConstructor create(java.lang.reflect.Constructor<?> constructor) {
        ...
        // Constructor passing along the Java Reflection Method
        {
            ...
            mv.visitMethodInsn(INVOKESPECIAL, "com/bergerkiller/mountiplex/reflection/util/fast/GeneratedConstructor", "<init>", "(Ljava/lang/reflect/Constructor;)V");
            mv.visitInsn(RETURN);
            mv.visitMaxs(2, 2);
            mv.visitEnd();
        }
        ...
        // invokeVA proxy method delegating to the correct method for invocation after checking args length
        {
            ...
            }
            {
                ...
                mv.visitVarInsn(ALOAD, 1);
                mv.visitInsn(ARRAYLENGTH);
                mv.visitMethodInsn(INVOKEVIRTUAL, Type.getInternalName(GeneratedConstructor.class), "failArgs", "(I)Ljava/lang/RuntimeException;");
                mv.visitInsn(ATHROW);
            }
